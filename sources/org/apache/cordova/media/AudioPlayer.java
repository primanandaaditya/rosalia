...
public class AudioPlayer implements OnCompletionListener, OnPreparedListener, OnErrorListener {
    ...
    public void moveFile(String file) {
        ...
        if (!file.startsWith("/")) {
            if (Environment.getExternalStorageState().equals("mounted")) {
                ...
                file = "/data/data/" + this.handler.cordova.getActivity().getPackageName() + "/cache/" + file;
            }
        }
    ...
    private void setState(STATE state2) {
        if (this.state != state2) {
            sendStatusChange(MEDIA_STATE, null, Float.valueOf((float) state2.ordinal()));
        }
        this.state = state2;
    }
    ...
    private void sendStatusChange(int messageType, Integer additionalCode, Float value) {
        if (additionalCode == null || value == null) {
            ...
            try {
                statusDetails.put("id", this.f17id);
                statusDetails.put("msgType", messageType);
                if (additionalCode != null) {
                    JSONObject code = new JSONObject();
                    code.put("code", additionalCode.intValue());
                    statusDetails.put(Param.VALUE, code);
                } else if (value != null) {
                    statusDetails.put(Param.VALUE, (double) value.floatValue());
                }
