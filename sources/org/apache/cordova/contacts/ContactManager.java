...
public class ContactManager extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext2) throws JSONException {
        ...
        if (VERSION.RELEASE.startsWith("1.")) {
            callbackContext2.sendPluginResult(new PluginResult(Status.ERROR, 5));
            return true;
        }
        if (this.contactAccessor == null) {
            this.contactAccessor = new ContactAccessorSdk5(this.cordova);
        }
        if (action.equals(Event.SEARCH)) {
        ...
        this.cordova.getThreadPool().execute(new Runnable() {
            public void run() {
                if (ContactManager.this.contactAccessor.remove(contactId)) {
                    ...
                    ContactManager.this.callbackContext.sendPluginResult(new PluginResult(Status.ERROR, 0));
                }
            }
        });
    }
    ...
    private void pickContactAsync() {
        this.cordova.getThreadPool().execute(new Runnable() {
            public void run() {
                this.cordova.startActivityForResult(this, new Intent("android.intent.action.PICK", Contacts.CONTENT_URI), 1000);
            }
        });
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (resultCode == -1) {
            ...
            Cursor c = this.cordova.getActivity().getContentResolver().query(RawContacts.CONTENT_URI, new String[]{"_id"}, "contact_id = " + contactId, null, null);
            if (!c.moveToFirst()) {
