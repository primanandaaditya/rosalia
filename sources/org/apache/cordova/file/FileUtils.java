...
public class FileUtils extends CordovaPlugin {
            ...
            if (!installedFileSystems.contains(fsName)) {
                ...
                if (fsRoot != null) {
                    ...
                    if (newRoot.mkdirs() || newRoot.isDirectory()) {
                        ...
                        LOG.m19d(LOG_TAG, "Unable to create root dir for filesystem \"" + fsName + "\", skipping");
                    }
                } else {
                    LOG.m19d(LOG_TAG, "Unrecognized extra filesystem identifier: " + fsName);
                }
            }
        }
    }
    ...
    public HashMap<String, String> getAvailableFileSystems(Activity activity) {
        ...
        if (Environment.getExternalStorageState().equals("mounted")) {
            try {
                ...
                LOG.m19d(LOG_TAG, "External storage unavailable, check to see if USB Mass Storage Mode is on");
            }
        }
    ...
    public void initialize(CordovaInterface cordova, CordovaWebView webView) {
        ...
        LOG.m22e(LOG_TAG, "File plugin configuration error: Please set AndroidPersistentFileLocation in config.xml to one of \"internal\" (for new applications) or \"compatibility\" (for compatibility with previous versions)");
        activity.finish();
    }
    ...
    public boolean execute(String action, final String rawArgs, final CallbackContext callbackContext) {
        if (!this.configured) {
            ...
            threadhelper(new FileOp() {
                public void run(JSONArray args) {
                    callbackContext.sendPluginResult(new PluginResult(Status.OK, (float) DirectoryManager.getFreeExternalStorageSpace()));
                }
            }, rawArgs, callbackContext);
            return true;
            ...
            threadhelper(new FileOp() {
                public void run(JSONArray args) throws JSONException, FileNotFoundException, IOException, NoModificationAllowedException {
                    ...
                    callbackContext.sendPluginResult(new PluginResult(Status.OK, (float) FileUtils.this.write(fname, data, offset, isBinary.booleanValue())));
                }
            }, rawArgs, callbackContext);
            return true;
    ...
    public JSONObject requestAllPaths() throws JSONException {
        ...
        if (Environment.getExternalStorageState().equals("mounted")) {
            try {
                ...
                LOG.m19d(LOG_TAG, "Unable to access these paths, most liklely due to USB storage");
            }
        }
        return ret;
    }
    ...
    public void readFileAs(String srcURLstr, int start, int end, final CallbackContext callbackContext, final String encoding, final int resultType) throws MalformedURLException {
        try {
            ...
            fs.readFileAtURL(inputURL, (long) start, (long) end, new ReadFileCallback() {
                public void handleData(InputStream inputStream, String contentType) {
                    ...
                    try {
                        ...
                        LOG.m19d(FileUtils.LOG_TAG, e.getLocalizedMessage());
            ...
            LOG.m19d(LOG_TAG, e3.getLocalizedMessage());
    ...
    public void onRequestPermissionResult(int requestCode, String[] permissions2, int[] grantResults) throws JSONException {
        final Request req = this.pendingRequests.getAndRemove(requestCode);
        if (req != null) {
            ...
            switch (req.getAction()) {
                ...
                case 1:
                    threadhelper(new FileOp() {
                        public void run(JSONArray args) throws JSONException, FileNotFoundException, IOException, NoModificationAllowedException {
                            req.getCallbackContext().sendPluginResult(new PluginResult(Status.OK, (float) FileUtils.this.write(args.getString(0), args.getString(1), args.getInt(2), Boolean.valueOf(args.getBoolean(3)).booleanValue())));
                        }
                    }, req.getRawArgs(), req.getCallbackContext());
                    return;
                case 2:
            ...
            LOG.m19d(LOG_TAG, "Received permission callback for unknown request code");
        }
    }
}
