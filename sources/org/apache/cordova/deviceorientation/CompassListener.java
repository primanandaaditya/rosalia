...
public class CompassListener extends CordovaPlugin implements SensorEventListener {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext2) throws JSONException {
        if (action.equals("start")) {
            ...
            callbackContext2.sendPluginResult(new PluginResult(Status.OK, getStatus()));
            return true;
        } else if (action.equals("getHeading")) {
            if (this.status != RUNNING) {
                if (start() == ERROR_FAILED_TO_START) {
                    callbackContext2.sendPluginResult(new PluginResult(Status.IO_EXCEPTION, ERROR_FAILED_TO_START));
                    return true;
                }
            ...
            callbackContext2.sendPluginResult(new PluginResult(Status.OK, getCompassHeading()));
            return true;
            ...
            callbackContext2.sendPluginResult(new PluginResult(Status.OK, (float) getTimeout()));
            return true;
        }
    }
    ...
    private JSONObject getCompassHeading() throws JSONException {
        ...
        obj.put(Param.TIMESTAMP, this.timeStamp);
        return obj;
    }
}
